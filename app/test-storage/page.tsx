...
export default function TestStoragePage() {
  ...
  const [userType, setUserTypeState] = useState<string>(getUserType() || 'guest');
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="container mx-auto max-w-4xl">
        ...
        <p className="text-muted-foreground mb-6">测试游客模式和登录模式的数据存储</p>
        
        <div className="mb-6 p-4 bg-secondary rounded-lg">
          <p className="font-semibold text-lg">
            当前模式: <span className="text-primary">{userType === 'guest' ? '🎮 游客模式' : '☁️ 登录模式'}</span>
          </p>
        ...
        <div className="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
          ...
          <ul className="text-sm space-y-1 text-muted-foreground">
            <li>• <strong>游客模式</strong>: 数据保存在浏览器 localStorage，关闭页面不丢失，但换设备会丢失</li>
            <li>• <strong>登录模式</strong>: 数据保存在云端数据库，可以跨设备同步</li>
            <li>• <strong>数据迁移</strong>: 将游客模式的本地数据上传到云端（登录后自动触发）</li>
            <li>• <strong>当前实现</strong>: 登录模式使用测试用户 ID (test-user-123)</li>
          </ul>
        </div>
      </div>
    </div>
